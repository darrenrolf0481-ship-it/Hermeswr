...
```bash
npm test              # 128 unit + integration tests, ~10s
npm run test:browser  # 101 assertions in real chromium, ~3min
```
...
| `test/a11y.test.ts` | Keyboard activation rules shared by controls that cannot be a native button, and the Tab-loop arithmetic that keeps focus inside a dialog. |
| `test/browser/reconMap.mjs` | Real-browser check of the panel, map geometry, a11y, dialog focus, phone layout and exports. |
